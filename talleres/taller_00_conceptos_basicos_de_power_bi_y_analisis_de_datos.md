# Taller 00. Conceptos básicos de Power BI y análisis de datos
...
Caso de estudio del taller: Una empresa necesita transformar datos en decisiones iniciales sobre el mercado inmobiliario.

## 2. Marco teórico / conceptual

### 2.1 Concepto central del taller

**Tema del taller:** Presentación del curso, reglas, metodología CRISP-DM y primer análisis descriptivo.

Este taller se orienta a que el estudiante entienda cómo los datos se convierten en información útil para decidir. El foco está en interpretar el problema, comprender las variables, construir indicadores y comunicar hallazgos. Administración: lectura gerencial del mercado de vivienda. Ingeniería Industrial: análisis básico de variables de producto y demanda.

### 2.2 Conceptos estadísticos que se aplican

**Concepto principal:** media, mediana y distribución básica.
...
- **Power BI:** carga de datos, vistas de datos/modelo/reporte, tarjetas, barras, filtros y segmentadores.
- **DAX:** AVERAGE, MAX, MIN, MEDIAN, IF, UPPER, CALCULATE, TODAY.
- **SQL:** SELECT, WHERE y ORDER BY a nivel conceptual.
- **Python:** lectura básica de CSV y revisión inicial con pandas.
...
**Dataset sugerido para el trabajo en clase:** Housing Prices Dataset

1. Abrir Power BI Desktop y registrar el problema de negocio que se va a resolver.
2. Cargar el dataset sugerido: **Housing Prices Dataset**.
...
6. Crear medidas y/o columnas DAX enfocadas en: medidas básicas de tendencia central, una columna lógica de clasificación, una medida con CALCULATE para comparar segmentos.
7. Diseñar visualizaciones con énfasis en: carga de datos, vistas de datos/modelo/reporte, tarjetas, barras, filtros y segmentadores.
...
- ¿qué variables parecen influir en el precio?
- ¿cómo cambia el precio promedio por número de habitaciones?
- ¿se observan diferencias por vecindario?

### Operaciones mínimas que deben implementarse

- medidas básicas de tendencia central
- una columna lógica de clasificación
- una medida con CALCULATE para comparar segmentos
...
**Nuevo contexto:** una empresa de mercadeo directo quiere identificar perfiles de clientes con mayor gasto.

**Dataset del reto:** DirectMarketing.csv.
